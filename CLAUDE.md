...
- **MCP session expires** after ~10 min inactivity — run `/mcp reconnect aap` to reconnect.
- **`.mcp.json` uses env vars** (`${AAP_MCP_URL}`, `${AAP_TOKEN}`) — run `source .env` before launching Claude Code or MCP auth will fail.
